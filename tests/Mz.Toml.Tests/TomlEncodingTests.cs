...
public sealed class TomlEncodingTests
{
    ...
    {
        var bytes = new UTF8Encoding(false).GetBytes("name = \"MÃ¤ÃŸig æ—¥æœ¬èªž\"\n");

        var result = Toml.TryParse(bytes);

        Assert.True(result.IsSuccess);
        Assert.Equal("MÃ¤ÃŸig æ—¥æœ¬èªž", result.Document.Root.AsValue("name").AsString());
    }

    [Fact]
    public void TryParse_Bytes_Accepts_Single_Leading_Utf8_Bom()
    {
        var payload = new UTF8Encoding(false).GetBytes("value = 1\n");
        ...
        Assert.True(result.IsSuccess);
        Assert.Equal(1, result.Document.Root.AsValue("value").AsInteger());
    }
    ...
    public void TryParse_Bytes_Rejects_Invalid_Utf8()
    {
        ...
        Assert.Equal(TomlDiagnosticCode.InvalidEncoding, result.Diagnostics[0].Code);
    }
    ...
    public void Second_Utf8_Bom_Is_Not_Treated_As_Encoding_Preamble()
    {
        ...
        Assert.False(result.IsSuccess);
        Assert.NotEqual(TomlDiagnosticCode.InvalidEncoding, result.Diagnostics[0].Code);
    }
    ...
    public void Literal_Replacement_Character_Remains_Valid_Unicode()
    {
        ...
        var result = Toml.TryParse(bytes);
        Assert.True(result.IsSuccess);
        Assert.Equal("\uFFFD", result.Document.Root.AsValue("value").AsString());
    }
    ...
    public void Parse_Bytes_Throws_TomlParseException_For_Invalid_Utf8()
    {
        ...
        Assert.Equal(TomlDiagnosticCode.InvalidEncoding, exception.Diagnostic.Code);
    }

    [Fact]
    public void Null_Byte_Input_Throws_ArgumentNullException()
    {
        Assert.Throws<ArgumentNullException>(() => Toml.TryParse((byte[])null!));
        Assert.Throws<ArgumentNullException>(() => Toml.Parse((byte[])null!));
    }

    [Fact]
    public void String_Api_Remains_Already_Decoded_Unicode_API()
    {
        var result = Toml.TryParse("value = \"\uFFFD\"\n");
