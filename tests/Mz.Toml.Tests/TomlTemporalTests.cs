...
namespace Mz.Toml.Tests
{
    public sealed class TomlTemporalTests
    {
        ...
        public void Offset_DateTime_Accepts_Toml_10_Separators(
            string text)
        {
            var document =
                Toml.Parse(
        ...
        public void Parses_Local_DateTime()
        {
            ...
            Assert.Equal(
                "555",
                temporal.Time.FractionalSeconds);
        }
        ...
        public void Preserves_Arbitrary_Fractional_Precision()
        {
            var document =
                Toml.Parse(
                    "value = 10:32:00.12345678901234567890\n");
        ...
        public void Local_Date_Supports_Toml_Year_Bounds(
            string text)
        {
            Assert.True(
                Toml.TryParse(
        ...
        public void Invalid_Local_Dates_Are_Rejected(
            string text)
        {
            Assert.False(
                Toml.TryParse(
        ...
        [InlineData("01:00:0")]
        [InlineData("01:00:00.")]
        public void Invalid_Local_Times_Are_Rejected(
            string text)
        {
            Assert.False(
                Toml.TryParse(
        ...
        [InlineData("-23:59", -1439)]
        public void Supports_Full_Toml_Numeric_Offset_Field_Range(
            string offset,
            int expectedMinutes)
        {
            var value =
        ...
        [InlineData("+0100")]
        [InlineData("+01:0")]
        public void Invalid_Offsets_Are_Rejected(
            string offset)
        {
            Assert.False(
                Toml.TryParse(
        ...
        public void Toml_11_Optional_Seconds_Remain_Rejected(
            string text)
        {
            Assert.False(
                Toml.TryParse(
        ...
        public void Temporal_Values_Work_Inside_Inline_Table()
        {
            var document =
                Toml.Parse(
                    "value = { date = 2006-06-01, time = 11:00:00 }\n");
            ...
            Assert.Equal(
                TomlValueKind.LocalDate,
                Assert.IsType<TomlValue>(
                    table["date"]).ValueKind);

            Assert.Equal(
                TomlValueKind.LocalTime,
                Assert.IsType<TomlValue>(
                    table["time"]).ValueKind);
        }
