...
namespace Mz.ApiProtocol.Tests
{
    public sealed class ApiDiscoveryWireProtocolTests
    {
        ...
        public void CreateRequest_UsesVersionedTransportSafeFields()
        {
            ...
            Assert.Equal(ApiDiscoveryWireProtocol.RequestMarker, fields[0]);
            Assert.Equal(ApiProtocolInfo.WireProtocolVersion.ToString(), fields[1]);
            Assert.Equal(ApiProtocolInfo.LibraryVersion.ToString(), fields[2]);
            ...
            Assert.Equal((int)ApiDependencyKind.Optional, fields[9]);
            Assert.Equal("Adds Command API integration", fields[10]);
        ...
        public void TryParseRequest_ValidPayload_ReturnsRequest()
        {
            ...
            bool success = ApiDiscoveryWireProtocol.TryParseRequest(payload, out ApiDiscoveryRequest request);
        ...
        public void TryParseRequest_AdditionalFields_AreIgnored()
        {
            ...
            Assert.NotNull(request);
            Assert.Equal(correlationId, request.CorrelationId);
            Assert.Equal("Mz.ConsumerMod", request.Dependency.Consumer.Id);
            Assert.Equal("Mz.CommandAPI", request.Dependency.Requirement.ApiId);
        }

        [Theory]
        [MemberData(nameof(GetInvalidRequestPayloads))]
        public void TryParseRequest_InvalidPayload_ReturnsFalse(object? payload)
        {
            bool success = ApiDiscoveryWireProtocol.TryParseRequest(payload!, out ApiDiscoveryRequest request);
        ...
        public void CreateAnnouncement_UsesVersionedTransportSafeFields()
        {
            ...
            Assert.Equal(ApiDiscoveryWireProtocol.AnnouncementMarker, fields[0]);
            Assert.Equal(ApiProtocolInfo.WireProtocolVersion.ToString(), fields[1]);
            Assert.Equal(ApiProtocolInfo.LibraryVersion.ToString(), fields[2]);
            ...
            var endpoints = Assert.IsType<Dictionary<string, Delegate>>(fields[10]);

            Assert.Same(endpoint, endpoints["RegisterCommand"]);
        }
        ...
        public void TryParseAnnouncement_ValidPayload_ReturnsAnnouncement()
        {
            ...
            Assert.Equal("Mz.CommandAPI", announcement.Descriptor.ApiId);
            Assert.Equal(new(1, 2, 3), announcement.Descriptor.Version);
            Assert.Equal(providerInstanceId, announcement.ProviderInstanceId);
            Assert.Equal(correlationId, announcement.CorrelationId);
            Assert.Equal(ApiProtocolInfo.WireProtocolVersion, announcement.WireProtocolVersion);
            Assert.Equal(ApiProtocolInfo.LibraryVersion, announcement.LibraryVersion);
            Assert.Same(endpoint, announcement.Endpoints["RegisterCommand"]);
        }

        [Fact]
        public void TryParseAnnouncement_AllowsUnsolicitedAnnouncement()
        {
            object payload = ApiDiscoveryWireProtocol.CreateAnnouncement(
                CreateProviderIdentity(),
                CreateDescriptor(),
                Guid.NewGuid(),
                Guid.Empty,
            ...
            bool success = ApiDiscoveryWireProtocol.TryParseAnnouncement(
                payload,
        ...
        public void TryParseAnnouncement_AdditionalFields_AreIgnored()
        {
            ...
            Assert.NotNull(announcement);
            Assert.Equal(providerInstanceId, announcement.ProviderInstanceId);
            Assert.Equal("Mz.CommandApiMod", announcement.Provider.Id);
            Assert.Equal("Mz.CommandAPI", announcement.Descriptor.ApiId);
        }
        ...
        public void TryParseAnnouncement_InvalidPayload_ReturnsFalse(object? payload)
        {
            bool success = ApiDiscoveryWireProtocol.TryParseAnnouncement(payload!, out ApiAnnouncement announcement);
        ...
        public void CreateWithdrawal_UsesVersionedTransportSafeFields()
        {
            ...
            object payload = ApiDiscoveryWireProtocol.CreateWithdrawal(CreateProviderIdentity(), "Mz.CommandAPI", providerInstanceId);

            object[] fields = Assert.IsType<object[]>(payload);

            Assert.Equal(8, fields.Length);
            Assert.Equal(ApiDiscoveryWireProtocol.WithdrawalMarker, fields[0]);
            Assert.Equal(ApiProtocolInfo.WireProtocolVersion.ToString(), fields[1]);
            Assert.Equal(ApiProtocolInfo.LibraryVersion.ToString(), fields[2]);
            Assert.Equal("Mz.CommandApiMod", fields[3]);
            Assert.Equal("Command API", fields[4]);
        ...
        public void TryParseWithdrawal_ValidPayload_ReturnsWithdrawal()
        {
            ...
            bool success = ApiDiscoveryWireProtocol.TryParseWithdrawal(payload, out ApiProviderWithdrawal withdrawal);

            Assert.True(success);
            Assert.NotNull(withdrawal);
            Assert.Equal("Mz.CommandAPI", withdrawal.ApiId);
            Assert.Equal(providerInstanceId, withdrawal.ProviderInstanceId);
            Assert.Equal(ApiProtocolInfo.WireProtocolVersion, withdrawal.WireProtocolVersion);
            Assert.Equal(ApiProtocolInfo.LibraryVersion, withdrawal.LibraryVersion);
        }
        ...
        public void TryParseWithdrawal_AdditionalFields_AreIgnored()
        {
            ...
            Assert.NotNull(withdrawal);
            Assert.Equal(providerInstanceId, withdrawal.ProviderInstanceId);
            Assert.Equal("Mz.CommandApiMod", withdrawal.Provider.Id);
            Assert.Equal("Mz.CommandAPI", withdrawal.ApiId);
        }

        [Theory]
        [MemberData(nameof(GetInvalidWithdrawalPayloads))]
        public void TryParseWithdrawal_InvalidPayload_ReturnsFalse(object? payload)
        {
            bool success = ApiDiscoveryWireProtocol.TryParseWithdrawal(payload!, out ApiProviderWithdrawal withdrawal);
        ...
        public static IEnumerable<object?[]> GetInvalidAnnouncementPayloads()
        {
            yield return [null];
            yield return ["not an array"];
            yield return [Array.Empty<object>()];
            yield return [CreateAnnouncementFields(marker: "wrong marker")];
            yield return [CreateAnnouncementFields(wireVersion: "invalid")];
            yield return [CreateAnnouncementFields(libraryVersion: "invalid")];
            yield return [CreateAnnouncementFields(apiId: "")];
            yield return [CreateAnnouncementFields(apiVersion: "invalid")];
            yield return [CreateAnnouncementFields(providerInstanceId: Guid.Empty)];
            ...
            yield return [CreateAnnouncementFields(endpoints: new Dictionary<string, Delegate> { { "Invalid", null! } })];
        }

        public static IEnumerable<object?[]> GetInvalidWithdrawalPayloads()
        {
            yield return [null];
            yield return ["not an array"];
            yield return [Array.Empty<object>()];
        ...
        private static object[] CreateAnnouncementFields(
            string marker = ApiDiscoveryWireProtocol.AnnouncementMarker,
        ...
        )
        {
            ...
            [
                ...
                endpoints ?? new Dictionary<string, Delegate>()
            ];
        }

        private static ApiDescriptor CreateDescriptor() 
            => new("Mz.CommandAPI", new(1, 2, 3));
