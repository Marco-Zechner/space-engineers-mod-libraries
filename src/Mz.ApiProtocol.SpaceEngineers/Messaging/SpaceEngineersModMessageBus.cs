...
namespace Mz.ApiProtocol.SpaceEngineers
{
    ...
    /// </summary>
    public sealed class SpaceEngineersModMessageBus : IModMessageBus
    {
        /// <inheritdoc />
        public void RegisterHandler(long channelId, Action<object> handler)
        {
            ...
            GetUtilities().RegisterMessageHandler(channelId, handler);
        }
        ...
        public void UnregisterHandler(long channelId, Action<object> handler)
        {
            ...
            GetUtilities().UnregisterMessageHandler(channelId, handler);
        }

        /// <inheritdoc />
        public void Send(long channelId, object payload)
            => GetUtilities().SendModMessage(channelId, payload);
        ...
        private static IMyUtilities GetUtilities()
        {
            ...
            if (utilities == null)
                throw new InvalidOperationException(
                    "Space Engineers utilities are unavailable. Use the mod-message bus during the active session lifecycle.");
